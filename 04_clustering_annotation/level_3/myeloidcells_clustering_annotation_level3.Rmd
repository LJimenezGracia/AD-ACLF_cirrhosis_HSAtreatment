...
# Load Seurat object
seurat_obj <- readRDS(paste0(path_r_objects, "/CLARIAJOA_clustering_level2_myeloidcells.rds"))
seurat_obj_proliferativecells <- readRDS(paste0(path_r_objects, "/CLARIAJOA_clustering_level2_mixedcells_myeloidcells.rds"))

seurat_obj <- merge(seurat_obj, seurat_obj_proliferativecells)
...
openxlsx::write.xlsx(biomarkers_list, file = paste0(path_r_tables, "/CLARIAJOA_myeloidcells_biomarkers_level3_res0.4.xlsx"))
```
...
macro_genes <- c("MARCO", "SPP1", "APOE", "C1QA", "C1QB",
                "CD68", "CD1630", "CD86", "MRC1") 
...
AXL_DC_genes <- c("SIGLEC6", "AXL", "CD22", "CD1C") 

FeaturePlot(seurat_obj_integrated,
            features = AXL_DC_genes,
            #split.by = "therapy",
            order = T,
            pt.size = 0.5,
            ncol = 4
...
```{r fig.width=15, fig.height=10}
DC4_genes <- c("FCGR3A", "FCGR3B", "FTL", "SERPINA1", "LST1", "IFITM2")
FeaturePlot(seurat_obj_integrated,
            features = DC4_genes,
            #split.by = "therapy",
            pt.size = 0.25,
...
```{r fig.width=20, fig.height=15}
# DCreg or TolDC
dysfunctional <- c("ARG1", "IDO1", "IL10", "TGFB1", "CD279", "CD274", "PTGES2") 
...
```{r fig.width=15, fig.height=5}
cycling <- c("CST7", "RETN", "S100A8", "S100A9", "S100A12") 
...
```{r fig.width=20, fig.height=10}
#https://www.ncbi.nlm.nih.gov/pmc/articles/PMC8019679/
#https://www.nature.com/articles/s41577-020-00490-y

  
G_MDSC_genes <- c("MMP9", "MMP8", "S100A8", "S100A9", "S100A12", "LCN2", "NAMPT", "LFT", "IFITM2", "FAU") 

FeaturePlot(seurat_obj_integrated,
            features = G_MDSC_genes,
            #split.by = "therapy",
            #order = T,
            pt.size = 0.5,
            ncol = 4
...
M_MDSC_genes <- c("VCAN", "LYZ", "CSTA", "S100A10", "CTSS", "CST3", "AIF1", "TMSB10", "FTL", "FCN1", "TYROBP") 

FeaturePlot(seurat_obj_integrated,
            features = M_MDSC_genes,
            #split.by = "therapy",
            #order = T,
            pt.size = 0.5,
            ncol = 4
...
E_MDSC_genes <- c("DEFA3", "MPO", "PRTN3", "DEFA4", "AZU1", "ELANE", "SLPI", "MS4A3", "RETN", "FTH1", "CEBPE") 

FeaturePlot(seurat_obj_integrated,
            features = E_MDSC_genes,
            #split.by = "therapy",
            #order = T,
            pt.size = 0.5,
            ncol = 4
...
seurat_obj_integrated <- FindSubCluster(
  ...
  cluster = "5",
  graph.name = "RNA_snn",
  subcluster.name = "RNA_snn_res.0.4_sub5",
  resolution = 0.1
)
...
```{r fig.width=15, fig.height=5}
genes <- c("CD14", "FCGR3A", "IFITM2", "AIF1") 
...
seurat_obj_integrated$annot_level3[seurat_obj_integrated$annot_level3 == "5"] <- as.character(
  seurat_obj_integrated$RNA_snn_res.0.4_sub5[seurat_obj_integrated$annot_level3 == "5"])
```
...
seurat_obj_integrated <- FindSubCluster(
  ...
  cluster = "4",
  graph.name = "RNA_snn",
  subcluster.name = "RNA_snn_res.0.4_sub4",
  resolution = 0.6
)
...
```{r fig.width=7, fig.height=5}
# Replacing new subclusters labels
seurat_obj_integrated$annot_level3[seurat_obj_integrated$annot_level3 == "4"] <- as.character(
  seurat_obj_integrated$RNA_snn_res.0.4_sub4[seurat_obj_integrated$annot_level3 == "4"])
```
...
seurat_obj_integrated <- FindSubCluster(
  ...
  cluster = "3",
  graph.name = "RNA_snn",
  subcluster.name = "RNA_snn_res.0.4_sub3",
  resolution = 0.2
)
...
```{r fig.width=7, fig.height=5}
# Replacing new subclusters labels
seurat_obj_integrated$annot_level3[seurat_obj_integrated$annot_level3 == "3"] <- as.character(
  seurat_obj_integrated$RNA_snn_res.0.4_sub3[seurat_obj_integrated$annot_level3 == "3"])
```
...
openxlsx::write.xlsx(biomarkers_list, file = paste0(path_r_tables, "/CLARIAJOA_myeloidcells_biomarkers_level3_res0.4_subclustering.xlsx"))
```
...
# Rename all identities
 seurat_obj_integrated <- RenameIdents(
  seurat_obj_integrated,
   ...
   "1" = "Intermediate Monocytes HAVCR2+",
   "2" = "Intermediate Monocytes Activated",
   "3_0" = "DC4 FCGR3B+",
     "3_1" = "G-MDSC CST7+",
     "3_2" = "DC4 FCGR3B+",
   "4_0" = "Non-Classical Monocytes",
     "4_1" = "Non-Classical Monocytes",
     "4_2" = "Non-Classical Monocytes",
   "4_3" = "Macrophages-like Monocytes",
   "5_0" = "Complex Platelets-Classical Monocytes",
    "5_1" = "Complex Platelets-Non-Classical Monocytes",
    "5_2" = "Complex Platelets-DCs",
  "6" = "Low quality cells",
  "7" = "G-MDSC",
  "8" = "Intermediate Monocytes IFN+",
  "9" = "DC2",
  "10" = "Intermediate Monocytes Inflammatory",
  "11" = "pDCs",
  "12" = "GMP cells"
  )
...
ggsave(filename = paste0(path_r_figs, "/CLARIAJOA_clustering_myeloidcells_level3_annotation_umap.png"),
...
saveRDS(seurat_obj_integrated@meta.data[c("annot_level1", "annot_level2", "annot_level3")],
        paste0(path_r_objects, "/CLARIAJOA_myeloidcells_annotation_level3.rds"))

# Save processed lognorm Seurat object
saveRDS(seurat_obj_integrated, paste0(path_r_objects, "/CLARIAJOA_myeloidcells_clustering_level3_annotation.rds"))
#seurat_obj_integrated <- readRDS(paste0(path_r_objects, "/CLARIAJOA_myeloidcells_clustering_level3_annotation.rds"))
```

```{r}
# Para Juan
seurat_obj_integrated <- DietSeurat(seurat_obj_integrated, counts = FALSE, data = TRUE, scale.data = FALSE, dimreducs = "umap")
...
saveRDS(seurat_obj_integrated, paste0(path_r_objects, "/CLARIAJOA_myeloidcells_level3_Juan.rds"))
```
...
ggsave(filename = paste0(path_r_figs, "/CLARIAJOA_clustering_myeloidcells_level3_annotation_umap_cleaned.png"),
       plot = gg_umap_clustering_annot)
```
...
# Save annotations
saveRDS(seurat_obj_integrated@meta.data[c("annot_level1", "annot_level2", "annot_level3")],
        paste0(path_r_objects, "/CLARIAJOA_myeloidcells_annotation_level3_cleaned.rds"))

# Save processed lognorm Seurat object
saveRDS(seurat_obj_integrated, paste0(path_r_objects, "/CLARIAJOA_myeloidcells_clustering_level3_annotation_cleaned.rds"))
#seurat_obj_integrated <- readRDS(paste0(path_r_objects, "/CLARIAJOA_myeloidcells_clustering_level3_annotation_cleaned.rds"))
```
...
# Remove uninteresting cells
Idents(seurat_obj_integrated) <- "annot_level3"
cells_to_remove <- c("Complex Platelets-Classical Monocytes",
...
gg_umap_clustering_annot <- DimPlot(object = seurat_obj_integrated_subset,
  ...
  labs(title = "Myeloid cells - level3 (subset)") +
  guides(color = guide_legend(override.aes = list(size = 5), ncol = 1))
gg_umap_clustering_annot
```
...
```{r}
# Save processed lognorm Seurat object
saveRDS(seurat_obj_integrated_subset, paste0(path_r_objects, "/CLARIAJOA_myeloidcells_clustering_level3_annotation_cleaned_subset.rds"))
#seurat_obj_integrated_subset <- readRDS(paste0(path_r_objects, "/CLARIAJOA_myeloidcells_clustering_level3_annotation_cleaned_subset.rds"))
```
