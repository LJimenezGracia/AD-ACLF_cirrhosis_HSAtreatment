---
title: "Data normalization and pre-processing - CD8+ T cells"
...
# Load Seurat object
seurat_obj_Tcells <- readRDS(paste0(path_r_objects, "/CLARIAJOA_clustering_level3_CD8Tcells.rds"))
seurat_obj_cd8cells1 <- readRDS(paste0(path_r_objects, "/CLARIAJOA_clustering_level3_CD4Tcells_CD8Tcells.rds"))
seurat_obj_cd8cells2 <- readRDS(paste0(path_r_objects, "/CLARIAJOA_clustering_level3_NKcells_CD8Tcells.rds"))

seurat_obj <- merge(seurat_obj_Tcells, c(seurat_obj_cd8cells1, seurat_obj_cd8cells2))

rm(seurat_obj_Tcells)
rm(seurat_obj_cd8cells1)
rm(seurat_obj_cd8cells2)
...
Idents(seurat_obj_integrated) <- "RNA_snn_res.1"
...
openxlsx::write.xlsx(biomarkers_list, file = paste0(path_r_tables, "/CLARIAJOA_CD8Tcells_biomarkers_level4_res1.xlsx"))
```
...
memoryT_genes <- c("KLRB1", "KLRG1", "IL7R", "PRDM1", "PDCD1", "CCR7", "IL3RA", "EGR1", "ICOS", "PASK", "CD69", "FAS", "IL12RB1", "RORC")
...
```{r fig.width=14, fig.height=10}
gdT_Vdelta1_genes <- c("ALOX65AP", "FCGR3A", "KLRC2", "CCL3", "KLRF1", "FGFBP2", "TYROBP", "CMC1", "TRGC2", "GZMB", "GZMH", "PLAC8", "GNLY")
gdT_Vdelta2_genes <- c("KLRC1", "LTB", "CD44", "CD52", "CD74", "LAPTM5", "KLRB1", "EIF3E", "LYAR", "HINT1", "EEF2", "ANXA1")

FeaturePlot(seurat_obj_integrated,
            features = gdT_Vdelta1_genes,
            #split.by = "tissue",
            #order = T,
            pt.size = 0.5,
            ncol = 3
...
seurat_obj_integrated <- FindSubCluster(
  ...
  cluster = "7",
  graph.name = "RNA_snn",
  subcluster.name = "RNA_snn_res.1_sub7",
  resolution = 0.1
)
...
seurat_obj_integrated$annot_level4[seurat_obj_integrated$annot_level4 == "7"] <- as.character(
  seurat_obj_integrated$RNA_snn_res.1_sub7[seurat_obj_integrated$annot_level4 == "7"])
```
...
openxlsx::write.xlsx(biomarkers_list, file = paste0(path_r_tables, "/CLARIAJOA_CD8Tcells_biomarkers_level4_res1_subclustering.xlsx"))
```
...
seurat_obj_integrated <- RenameIdents(
 seurat_obj_integrated,
  "0" = "Naive CD8+ T cells", 
  "1" = "Transitional Memory CD8+ T cells",
  "2" = "Effector Memory CD8+ T cells",
  "3" = "MAIT cells",
  "4" = "Effector Activated CD8+ T cells",
  "5" = "NK-like T cells",
  "6" = "Central Memory CD8+ T cells",
  "7_0" = "Activated CD8+ T cells",
  "7_1" = "Proliferative CD8+ T cells",
  "8" = "gd T cells",
  "9" = "IFN+ CD8+ T cells",
   "10" = "Transitional Memory CD8+ T cells",
  "11" = "Innate-like T cells",  # TCR+ FCER1G+ innate-like T cells (?) https://www.nature.com/articles/s41392-022-01061-4
 "12" = "Proliferative CD8+ T cells",
 "13" = "dN HAVCR2+ T cells"
)
...
gg_umap_clustering_annot <- DimPlot(object = seurat_obj_integrated,
  ...
  labs(title = "CD8+ T cells - Level 4")
gg_umap_clustering_annot

ggsave(filename = paste0(path_r_figs, "/CLARIAJOA_clustering_CD8Tcells_level4_annotation_umap.png"),
...
saveRDS(seurat_obj_integrated@meta.data[c("annot_level1", "annot_level2", "annot_level3", "annot_level4")],
        paste0(path_r_objects, "/CLARIAJOA_CD8Tcells_annotation_level4.rds"))

# Save processed lognorm Seurat object
saveRDS(seurat_obj_integrated, paste0(path_r_objects, "/CLARIAJOA_CD8Tcells_clustering_level4_annotation.rds"))
#seurat_obj_integrated <- readRDS(paste0(path_r_objects, "/CLARIAJOA_CD8Tcells_clustering_level4_annotation.rds"))
```
...
seurat_metadata_variables <- c(names(seurat_obj_integrated@meta.data[1:38]), "annot_level1",  "annot_level2", "annot_level3", "annot_level4")
seurat_obj_integrated@meta.data <- seurat_obj_integrated@meta.data[seurat_metadata_variables]
saveRDS(seurat_obj_integrated, paste0(path_r_objects, "/CLARIAJOA_CD8Tcells_level4_Juan.rds"))
```
